...
class ConditioningTargetMixin:
            ...
            with self._load_model_context("vae"):
                ...
                for i in range(batch_size):
                    ...
                    if source_repaint_batch is not None:
                        ...
                        logger.info(
                            "[generate_music] Using source-session latents for repaint item {}...",
                            i,
                        )
    ...
    def _normalize_source_repaint_latents(
        ...
        """Normalize optional source-session latents to ``[B, T, C]``."""
        if source_repaint_latents is None:
            return None
        latents = torch.as_tensor(source_repaint_latents, device=self.device, dtype=self._get_vae_dtype())
        if latents.ndim == 2:
    ...
    def _align_source_repaint_latent(
        ...
        """Trim or silence-pad a source-session latent to the target length."""
